...
<template>
	<section class="mb-6 grid grid-cols-1 gap-4">
				...
				<div class="flex items-center mb-6">
					...
					<div>
						...
						<p class="text-sm font-normal text-gray-500 dark:text-gray-400">
							New leads; Retained leads; Leads categorized by current step as
							well as source
						</p>
					</div>
				</div>
				...
				<div class="flex flex-col gap-6 max-w-full overflow-x-auto py-4">
					<div>
						<div class="flex flex-row gap-6 ">
							<DataCard v-for="(avg, index) in averagesOutput" :key="index">
								...
								<template v-slot:content>
									<span>{{
										(avg.data ?? "__") != "__" ? `${avg.data} day(s)` : avg.data
									}}</span>
								</template>
							</DataCard>
						</div>
		...
		>
			...
			<div class="relative min-h-[550px]">
				<LeadBySource :dataSet="leadsBySource.data" :meta="leadsBySource.meta" />
			</div>
		</div>
	</section>
</template>
