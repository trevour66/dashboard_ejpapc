...
const parse_anticipate_funds = (anticipate_funds) => {
   try {
      dataCard_Data.anticipate_funds.data =
         anticipate_funds?.data?.count ?? "__";
...
const parse_atty_fees_by_responsible_atty = async (
   ...
   try {
      feesByResponsibleAtty.data =
         fees_by_responsible_atty?.data?.count ?? null;
      ...
      if (
         feesByResponsibleAtty?.data == null ||
         (feesByResponsibleAtty?.data ?? []).length == 0
      ) {
         throw new Error();
      }
   } catch (error) {
...
<template>
   <section class="mb-6 grid grid-cols-1 gap-4">
      ...
      <div class="grid grid-cols-1 gap-4">
         <div
            v-if="
               !loadingFeesByResponsibleAtty &&
               !errorLoadingFeesByResponsibleAtty
            "
