...
const init_getCasesStartupData_dashboard = async () => {
   ...
   const res =
      (await getCasesStartupData_dashboard(to, from, "count")) ?? false;
...
const parse_cases_with_next_step_overdue = (cases) => {
   try {
      dataCard_Data.cases_with_next_step_overdue.data =
         cases?.data?.count ?? "__";
      dataCard_Data.cases_with_next_step_overdue.meta.data =
         cases?.data ?? null;
...
const parse_cases_accessed_within_more_than_ten_day_ago = (cases) => {
   try {
      dataCard_Data.last_accessed_within_ten_days.data =
         cases?.data?.count ?? "__";
      dataCard_Data.last_accessed_within_ten_days.meta.data =
         cases?.data ?? null;
...
<template>
   <section class="mb-6 grid grid-cols-1 gap-4">
            ...
            <div class="flex items-center mb-6">
               ...
               <div>
                  ...
                  >
                     ...
                     Negotiations, Presettled, and Settled); Cases per
                     responsible attorney and Last Accessed Date( > 10 days)
                  </p>
               </div>
            </div>
      ...
      <div class="grid grid-cols-1 gap-4">
         ...
         >
            <CasesByStep
               :dataSet="casesBySteps.data"
               :meta="casesBySteps.meta"
            />
         </div>
         <div
