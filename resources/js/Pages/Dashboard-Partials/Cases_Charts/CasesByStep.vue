...
const tableHeaders = ref(["Step", "Count"]);
const tableRows = ref([]);
...
onMounted(() => {
   if ((props.dataSet ?? []).length > 0) {
      for (let index = 0; index < props.dataSet.length; index++) {
         ...
         const step_name = el?.step_name ?? false;
         const matter_count = el?.matter_count ?? 0;

         if (step_name) {
            dataSet_labels.value.push(step_name);
            // dataSet_series.value.push(matter_count);
            dataSet_series.value.push({
               x: step_name,
               y: matter_count,
            });

            tableRows.value.push([step_name, matter_count]);

            dataSet_total.value = dataSet_total.value + matter_count;
         }
      }
   }
});
